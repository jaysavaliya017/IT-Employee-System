...
const AttendanceDuration: React.FC<AttendanceDurationProps> = ({
  ...
  if (mode === 'detailed') {
    return (
      <div className="text-right">
        <div className="font-medium text-slate-800">{formatAttendanceDurationClock(attendance)}</div>
        <div className={`text-xs ${live ? 'text-green-600' : 'text-slate-500'}`}>
  ...
  return (
    <div>
      <div className="text-sm text-slate-800">{formatAttendanceHours(attendance)}</div>
