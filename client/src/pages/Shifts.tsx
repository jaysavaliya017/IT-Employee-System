...
const Shifts: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      notifyApiError(error, 'Could not save the shift');
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      notifyApiError(error, 'Could not delete the shift');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Shifts"
        subtitle="Working hours and grace periods"
        icon={Clock}
        actions={
          <button
            onClick={() => {
        ...
        <CardContent>
          {shifts.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {shifts.map((shift) => (
                <div key={shift.id} className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleEdit(shift)} className="p-2 hover:bg-slate-200 rounded">
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4">
            <button onClick={() => setShowModal(false)} className="px-4 py-2 bg-slate-100 rounded-lg">Cancel</button>
