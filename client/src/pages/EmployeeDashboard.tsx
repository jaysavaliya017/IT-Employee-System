      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3 shadow-sm">
          ...
          <div>
            ...
            <p className="font-semibold text-slate-800">{shift?.startTime ?? '09:00'}</p>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3 shadow-sm">
          ...
          <div>
            ...
            <p className="font-semibold text-slate-800">{shift?.endTime ?? '18:00'}</p>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3 shadow-sm">
          ...
          <div>
            <p className="text-xs text-slate-500">Leaves Used</p>
            <p className="font-semibold text-slate-800">{totalLeaveUsed} days</p>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3 shadow-sm">
          ...
          <div>
              <p className="text-xs text-slate-500">Holidays 2026</p>
              <p className="font-semibold text-slate-800">{upcomingHolidays.length} total</p>
          </div>
        </div>
      </div>
                      ...
                      <div key={lb.id}>
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className="text-sm text-slate-700">{lb.leaveType.name}</span>
                          </div>
                  ...
                  return (
                    ...
                    >
                      <div className={`w-12 h-12 rounded-lg flex flex-col items-center justify-center font-bold leading-tight shrink-0 ${
                        isToday ? 'bg-emerald-50 text-emerald-700' : isPast ? 'bg-slate-100 text-slate-400' : 'bg-amber-50 text-amber-700'
                      }`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className={`font-medium truncate ${isPast ? 'text-slate-500' : 'text-slate-800'}`}>
                        ...
                        <p className="text-xs text-slate-400 mt-1">{format(holidayDate, 'EEEE, MMMM d, yyyy')}</p>
                      </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-1.5">
                        {style.icon}
                        <p className="font-medium text-slate-800 text-sm">{ann.title}</p>
                      </div>
                      <span className={`text-xs px-1.5 py-0.5 rounded font-medium whitespace-nowrap ${style.badge}`}>
                        {ann.type}
                      </span>
                    </div>
                    <p className="text-xs text-slate-600 mt-1.5 leading-relaxed">{ann.message}</p>
