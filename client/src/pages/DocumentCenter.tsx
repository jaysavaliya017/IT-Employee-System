...
const DocumentCenter = () => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      notifyApiError(error, 'Could not delete the document');
    }
  };

  const getDocumentTypeBadge = (type: string) => {
    const types: Record<string, { label: string; color: string }> = {
      OFFER_LETTER: { label: 'Offer Letter', color: 'bg-emerald-50 text-emerald-700' },
      ...
      SALARY_SLIP: { label: 'Salary Slip', color: 'bg-amber-50 text-amber-700' },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Access company documents and policies</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm p-12 text-center">
          ...
          <p className="text-slate-500">Documents you have access to will appear here</p>
        </div>
      ) : (
