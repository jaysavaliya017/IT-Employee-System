...
const GalleryPage = () => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      notifyApiError(error, 'Could not delete the image');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Company Gallery</h1>
          <p className="text-slate-500 mt-1">Browse company photos and events</p>
        </div>
        <button
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-1 border border-slate-200 rounded-lg p-1">
            <button
              ...
              className={`p-2 rounded ${viewMode === 'grid' ? 'bg-primary-100 text-primary-600' : 'text-slate-400 hover:bg-slate-100'}`}
            >
              <Grid className="w-4 h-4" />
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`p-2 rounded ${viewMode === 'list' ? 'bg-primary-100 text-primary-600' : 'text-slate-400 hover:bg-slate-100'}`}
            >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {images.map((image) => (
            <div
              key={image.id}
              className="group relative bg-white rounded-xl shadow-sm overflow-hidden border border-slate-100 hover:shadow-md transition-shadow"
            >
              ...
              <div className="p-3">
                <h3 className="font-medium text-slate-900 truncate">{image.title}</h3>
                <p className="text-sm text-slate-500 truncate">{image.category || 'Uncategorized'}</p>
              </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {images.map((image) => (
                <tr key={image.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 font-medium text-slate-900">{image.title}</td>
                  <td className="px-6 py-4 text-slate-600">{image.category || '-'}</td>
                  <td className="px-6 py-4 text-slate-600">{new Date(image.createdAt).toLocaleDateString()}</td>
                  <td className="px-6 py-4">
      ...
      {selectedImage && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setSelectedImage(null)}>
          ...
          <div className="relative bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            />
            <div className="p-6">
              <h2 className="text-xl font-bold text-slate-900">{selectedImage.title}</h2>
              {selectedImage.description && (
                <p className="mt-2 text-slate-600">{selectedImage.description}</p>
              )}
              <div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-500">
                {selectedImage.category && <span className="px-2 py-1 bg-slate-100 rounded">Category: {selectedImage.category}</span>}
