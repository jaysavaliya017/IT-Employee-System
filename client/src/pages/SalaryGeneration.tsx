  ...
  const generateSalary = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || 'Could not generate salary');
      }
    } catch (error) {
      notifyApiError(error, 'Could not generate salary');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Salary Generation</h1>
          <p className="text-slate-500 mt-1">Generate monthly salary for employees</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Generate Monthly Salary</h2>
            <p className="text-sm text-slate-500">Create salary records for the selected period</p>
          </div>
        </div>
      ...
      {showResults && (
        <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-medium text-slate-700">Employee Code</th>
                  <th className="px-4 py-3 text-right font-medium text-slate-700">Basic</th>
                  <th className="px-4 py-3 text-right font-medium text-slate-700">Earnings</th>
                  <th className="px-4 py-3 text-right font-medium text-slate-700">Deductions</th>
                  <th className="px-4 py-3 text-right font-medium text-slate-700">Net Salary</th>
                  ...
                  <tr key={record.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      ...
                      {record.status === 'CREDITED' && (
                        <span className="px-3 py-1 text-xs font-medium rounded-full bg-emerald-50 text-emerald-700">Credited</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <button
          ...
          <div className="mt-8 pt-8 border-t border-slate-200">
            <h3 className="text-lg font-semibold text-slate-900 mb-4">Generation Status</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                    ...
                    <tr key={idx}>
                      ...
                      <td className="px-6 py-4">
                        {result.status === 'SUCCESS' ? (
                          <span className="px-2 py-1 text-xs font-medium rounded-full bg-emerald-50 text-emerald-700">Success</span>
                        ) : (
                          <span className="px-2 py-1 text-xs font-medium rounded-full bg-amber-50 text-amber-700">{result.status}</span>
                        )}
                      </td>
                      <td className="px-6 py-4 text-slate-600">{result.message}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-900 mt-1">{selectedRecord.employee?.fullName || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 font-medium">Employee Code</p>
                  <p className="text-sm font-mono font-semibold text-slate-900 mt-1">{selectedRecord.employee?.employeeCode || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 font-medium">Designation</p>
                  <p className="text-sm font-semibold text-slate-900 mt-1">{selectedRecord.employee?.designation || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 font-medium">Department</p>
                  <p className="text-sm font-semibold text-slate-900 mt-1">{selectedRecord.employee?.department?.name || 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="text-center">
                  <p className="text-xs text-slate-500 font-medium">Paid Days</p>
                  <p className="text-lg font-bold text-green-900 mt-1">{selectedRecord.paidDays || 0}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-slate-500 font-medium">Leave Days</p>
                  <p className="text-lg font-bold text-orange-900 mt-1">{selectedRecord.leaveDays || 0}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-slate-500 font-medium">Unpaid Days</p>
              ...
              <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.basicSalary)}</span>
                  </div>
                  {selectedRecord.hra > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-700">HRA</span>
                      <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.hra)}</span>
                    </div>
                  )}
                  {selectedRecord.conveyanceAllowance > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-700">Conveyance Allowance</span>
                      <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.conveyanceAllowance)}</span>
                    </div>
                  )}
                  {selectedRecord.medicalAllowance > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-700">Medical Allowance</span>
                      <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.medicalAllowance)}</span>
                    </div>
                  )}
                  {selectedRecord.specialAllowance > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-700">Special Allowance</span>
                      <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.specialAllowance)}</span>
                    </div>
                  )}
                  {selectedRecord.otherAllowances > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-700">Other Allowances</span>
                      <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.otherAllowances)}</span>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {selectedRecord.pfDeduction > 0 && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.pfDeduction)}</span>
                    </div>
                  )}
                  {selectedRecord.taxDeduction > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-700">Tax Deduction</span>
                      <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.taxDeduction)}</span>
                    </div>
                  )}
                  {selectedRecord.professionalTax > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-700">Professional Tax</span>
                      <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.professionalTax)}</span>
                    </div>
                  )}
                  {selectedRecord.leaveDeduction > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-700">Leave Deduction</span>
                      <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.leaveDeduction)}</span>
                    </div>
                  )}
                  {selectedRecord.otherDeductions > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-700">Other Deductions</span>
                      <span className="font-medium text-slate-900">{formatCurrency(selectedRecord.otherDeductions)}</span>
                    </div>
                  )}
                </div>
              </div>
                    ...
                    {selectedRecord.salaryComponents.map((comp, idx) => (
                      <div key={idx} className="flex justify-between text-sm">
                        <span className="text-slate-700">
                          {comp.componentName}
                          <span className="text-xs text-slate-500 ml-1">({comp.type})</span>
                        </span>
