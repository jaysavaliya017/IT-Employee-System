...
const AnnouncementList = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      notifyApiError(error, 'Could not save the announcement');
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      notifyApiError(error, 'Could not delete the announcement');
    }
  };
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'URGENT':
        return <span className="px-2 py-1 text-xs font-medium rounded-full bg-red-50 text-red-700 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> Urgent</span>;
      case 'HIGH':
        return <span className="px-2 py-1 text-xs font-medium rounded-full bg-orange-100 text-orange-700">High</span>;
      case 'MEDIUM':
        return <span className="px-2 py-1 text-xs font-medium rounded-full bg-amber-50 text-amber-700">Medium</span>;
      case 'LOW':
        return <span className="px-2 py-1 text-xs font-medium rounded-full bg-slate-100 text-slate-700">Low</span>;
      default:
        return null;
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">View and manage company announcements</p>
        </div>
        <button
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <p className="text-slate-500">Create your first announcement</p>
          </div>
        ) : (
          announcements.map((announcement) => (
            <div
              key={announcement.id}
              className={`bg-white rounded-xl shadow-sm p-6 border border-slate-100 ${isExpired(announcement.expiryDate) ? 'opacity-60' : ''}`}
            >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    {isExpired(announcement.expiryDate) && (
                      <span className="px-2 py-1 text-xs font-medium rounded-full bg-slate-100 text-slate-500">Expired</span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-4 mt-4 text-sm text-slate-500">
                    ...
                    <span className="px-2 py-1 bg-slate-100 rounded">{(announcement.visibilityType).replace('_', ' ')}</span>
                  </div>
                </div>
