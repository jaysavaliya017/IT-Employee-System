...
const NotificationPage = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-1 border border-slate-100 inline-flex">
        <button
          onClick={() => { setFilter('all'); setPagination((prev) => ({ ...prev, page: 1 })); }}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === 'all' ? 'bg-primary-100 text-primary-700' : 'text-slate-600 hover:bg-slate-100'}`}
        >
          All Notifications
        </button>
        <button
          onClick={() => { setFilter('unread'); setPagination((prev) => ({ ...prev, page: 1 })); }}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === 'unread' ? 'bg-primary-100 text-primary-700' : 'text-slate-600 hover:bg-slate-100'}`}
        >
          Unread ({unreadCount})
        </button>
      </div>
