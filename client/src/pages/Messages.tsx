...
const Messages: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      notifyApiError(error, 'Could not send the message');
    }
  };
  ...
  const handleSendBulk = async () => {
    ...
    try {
      ...
      notifyApiError(error, 'Could not send the reminder');
    }
  };
  ...
  return (
    <div className="h-[calc(100vh-6rem)] min-h-[700px] rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden flex">
      <aside className="w-[340px] border-r border-slate-100 flex flex-col">
        ...
        {bulkOpen && (
          <div className="p-4 border-b border-slate-100 space-y-2 bg-amber-50">
            <input
              value={bulkTitle}
        ...
        <div className="p-3 border-b border-slate-100">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-[0.16em]">Start Conversation</p>
          <div className="mt-2 max-h-28 overflow-y-auto space-y-1">
            {users.slice(0, 8).map((item) => (
              ...
              >
                <p className="text-sm font-medium text-slate-700">{item.fullName}</p>
            ...
            conversations.map((conversation) => {
              ...
              return (
                <button
                  ...
                  className={`w-full px-4 py-3 text-left border-b border-slate-50 hover:bg-slate-50 ${active ? 'bg-primary-50' : ''}`}
                >
      ...
      <main className="flex-1 flex flex-col">
        {!selectedConversation ? (
          ...
          <>
                ...
                {selectedConversation.participants
                  ...
                  .map((item) => (
                    <span key={item.user.id} className="inline-flex items-center gap-1">
                      <Circle
                        className={`w-2.5 h-2.5 ${onlineUsers[item.user.id] ? 'text-emerald-500 fill-emerald-500' : 'text-slate-300 fill-slate-300'}`}
                      />
            ...
            <section className="flex-1 overflow-y-auto p-5 space-y-3 bg-slate-50">
              {loadingMessages ? (
                ...
                messages.map((message) => {
                  ...
                  return (
                    <div key={message.id} className={`flex ${own ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] rounded-2xl px-4 py-3 shadow-sm ${own ? 'bg-primary-600 text-white' : 'bg-white text-slate-800'}`}>
                        <div className="flex items-center gap-2 mb-1">
                          <p className={`text-xs font-semibold ${own ? 'text-primary-100' : 'text-slate-500'}`}>{message.sender.fullName}</p>
                        ...
                        <p className={`mt-2 text-[10px] ${own ? 'text-primary-100' : 'text-slate-400'}`}>
            ...
            <footer className="border-t border-slate-100 p-4 space-y-3 bg-white">
              ...
              <div className="flex items-center gap-2">
                <label className="p-2 rounded-lg border border-slate-200 cursor-pointer hover:bg-slate-50">
