...
const SalarySlipList = () => {
  ...
  const creditSalary = async (recordId: string) => {
    try {
      ...
      toast.success('Salary marked as credited');
      fetchRecords();
    } catch (error) {
      notifyApiError(error, 'Could not credit the salary');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage and view all employee salary records</p>
        </div>
      </div>
                ...
                records.map((record) => (
                  <tr key={record.id} className="hover:bg-slate-50">
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-900">{record.employee?.fullName || 'N/A'}</div>
                      <div className="text-sm text-slate-500">{record.employee?.employeeCode || ''}</div>
                    </td>
