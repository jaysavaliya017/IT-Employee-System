...
const LeaveRequests: React.FC = () => {
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      notifyApiError(error, 'Could not approve the leave');
    }
  };
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      notifyApiError(error, 'Could not reject the leave');
    }
  };
                ...
                <div key={request.id} className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">{request.user?.employeeCode} | {request.user?.department?.name}</p>
                      <div className="mt-2 space-y-1">
                        <p className="text-sm"><span className="text-slate-500">Leave Type:</span> {request.leaveType?.name}</p>
