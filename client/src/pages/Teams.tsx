...
const Teams: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      notifyApiError(error, 'Could not save the team');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Teams"
        subtitle="Groups and their leaders"
        icon={Users}
        actions={
          <button
            onClick={() => {
        ...
        <CardContent>
          {teams.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {teams.map((team) => (
                <div key={team.id} className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{team.department?.name || 'No Department'}</p>
                      </div>
                    </div>
                    <button onClick={() => handleEdit(team)} className="p-2 hover:bg-slate-200 rounded">
