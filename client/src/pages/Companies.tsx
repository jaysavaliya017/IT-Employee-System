...
const Companies: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Companies"
        subtitle="Organizations using this workspace"
        icon={Building2}
        actions={
          ...
          {companies.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {companies.map((company) => (
                <div key={company.id} className="p-4 border border-slate-200 rounded-lg bg-white">
                  <div className="flex items-start justify-between">
                    ...
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${
                        company.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'
                      }`}
                    >
                  ...
                  <div className="mt-4 text-sm text-slate-600">
                    Users: <span className="font-medium text-slate-800">{company._count?.users || 0}</span>
                  </div>

                  <div className="mt-4 flex items-center gap-2">
                    <button
                      onClick={() => openEdit(company)}
                      className="flex-1 flex items-center justify-center gap-1 px-3 py-2 border border-slate-200 rounded-lg hover:bg-slate-50"
                    >
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="company-active" className="text-sm text-slate-700">
              Company is active
            </label>
          </div>
