...
async function main() {
  ...
  // Idempotency guard. The seed creates a lot of related rows (users, balances,
  // attendance, holidays) that are awkward to upsert individually. If the demo admin
  // already exists, the database has been seeded before, so skip and exit cleanly
  // instead of failing on a unique-constraint violation.
  const alreadySeeded = await prisma.user.findFirst({
    where: { email: 'admin@staffsync.com' },
    select: { id: true },
  });

  if (alreadySeeded) {
    console.log('Database already seeded (admin@staffsync.com exists). Skipping.');
    console.log('To reseed from scratch: npx prisma migrate reset');
    return;
  }

  const companies = [
    { name: 'CodeQuantum Technologies Private Limited', code: 'CQTPL' },
    { name: 'StaffSync Technologies', code: 'STAFFSYNC' },
  ...
  const defaultCompany = await prisma.company.findUnique({
    where: { code: 'STAFFSYNC' },
  });

  if (!defaultCompany) {
    throw new Error('Default company STAFFSYNC was not created');
  }
  ...
  const itDepartment = await prisma.department.upsert({
    where: { companyId_name: { companyId: defaultCompany.id, name: 'Information Technology' } },
    update: {},
    create: {
  ...
  const hrDepartment = await prisma.department.upsert({
    where: { companyId_name: { companyId: defaultCompany.id, name: 'Human Resources' } },
    update: {},
    create: {
      companyId: defaultCompany.id,
  ...
  const morningShift = await prisma.shift.upsert({
    where: { companyId_name: { companyId: defaultCompany.id, name: 'Morning Shift' } },
    update: {},
    create: {
      companyId: defaultCompany.id,
      name: 'Morning Shift',
  ...
  const eveningShift = await prisma.shift.upsert({
    where: { companyId_name: { companyId: defaultCompany.id, name: 'Evening Shift' } },
    update: {},
    create: {
      companyId: defaultCompany.id,
      name: 'Evening Shift',
  ...
  const paidLeave = await prisma.leaveType.upsert({
    where: { companyId_name: { companyId: defaultCompany.id, name: 'Paid Leave' } },
    update: {},
    create: {
      companyId: defaultCompany.id,
      name: 'Paid Leave',
  ...
  const sickLeave = await prisma.leaveType.upsert({
    where: { companyId_name: { companyId: defaultCompany.id, name: 'Sick Leave' } },
    update: {},
    create: {
      companyId: defaultCompany.id,
      name: 'Sick Leave',
  ...
  const casualLeave = await prisma.leaveType.upsert({
    where: { companyId_name: { companyId: defaultCompany.id, name: 'Casual Leave' } },
    update: {},
    create: {
      companyId: defaultCompany.id,
      name: 'Casual Leave',
  ...
  const unpaidLeave = await prisma.leaveType.upsert({
    where: { companyId_name: { companyId: defaultCompany.id, name: 'Unpaid Leave' } },
    update: {},
    create: {
      companyId: defaultCompany.id,
      name: 'Unpaid Leave',
  ...
  const workFromHome = await prisma.leaveType.upsert({
    where: { companyId_name: { companyId: defaultCompany.id, name: 'Work From Home' } },
    update: {},
    create: {
      companyId: defaultCompany.id,
      name: 'Work From Home',
  ...
  const superAdmin = await prisma.user.create({
    data: {
      ...
      email: 'superadmin@staffsync.com',
  ...
  const teamLeader = await prisma.user.create({
    data: {
      ...
      email: 'teamleader@staffsync.com',
  ...
  const developmentTeam = await prisma.team.upsert({
    where: { companyId_name_departmentId: { companyId: defaultCompany.id, name: 'Development Team', departmentId: itDepartment.id } },
    update: {},
    create: {
      companyId: defaultCompany.id,
  ...
  const qaTeam = await prisma.team.upsert({
    where: { companyId_name_departmentId: { companyId: defaultCompany.id, name: 'QA Team', departmentId: itDepartment.id } },
    update: {},
    create: {
      companyId: defaultCompany.id,
      name: 'QA Team',
  ...
  console.log('    Email: superadmin@staffsync.com');
  console.log('    Password: SuperAdmin@123');
  console.log('\n  HR/Admin:');
  console.log('    Email: admin@staffsync.com');
  console.log('    Password: Admin@123');
  console.log('\n  Manager:');
  console.log('    Email: manager@staffsync.com');
  console.log('    Password: Manager@123');
  console.log('\n  Team Leader:');
  console.log('    Email: teamleader@staffsync.com');
  console.log('    Password: Leader@123');
  console.log('\n  Employee:');
  console.log('    Email: employee@staffsync.com');
